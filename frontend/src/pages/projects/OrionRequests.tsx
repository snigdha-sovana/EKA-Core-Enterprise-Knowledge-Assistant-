...
export const OrionRequests: React.FC = () => {
  ...
  const orionRequests = requests.filter(
    r => r.project === 'Project Orion' || r.department === 'Projects'
  );

  const filteredRequests = orionRequests.filter(req => {
  ...
  const pendingCount = orionRequests.filter(r => r.status === 'Pending Review' || r.status === 'In Progress').length;
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <div className="text-xs font-semibold uppercase tracking-wider text-purple-600 mb-0.5">
            Technical Governance & Escalations
          </div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2">
            Technical Inquiry Escalation Handler
          </h1>
          <p className="text-xs text-slate-500">
            Principal Lead queue for architectural exceptions, AWS IAM STS permissions, and Kafka schema clarifications.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold text-purple-800 bg-purple-100 border border-purple-200 px-3 py-1.5 rounded-lg flex items-center gap-1.5">
            ...
            {orionRequests.length} Orion Technical Queries
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 enterprise-card p-4 bg-white">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search by Engineer, ID, or Query..."
            value={searchQuery}
        ...
        <div className="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
          {(['All', 'Pending Review', 'In Progress', 'Resolved'] as const).map(status => (
            ...
            >
              {status}
              {status === 'In Progress' && pendingCount > 0 && (
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 font-bold uppercase tracking-wider text-[10px] border-b border-slate-200/80">
                ...
                <th className="py-3 px-4">Technical Query</th>
              ...
              {filteredRequests.length === 0 ? (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-slate-400 text-xs">
                    No technical inquiries found matching the criteria.
                  </td>
                </tr>
              ) : (
                filteredRequests.map(req => (
                  <tr key={req.id} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right whitespace-nowrap">
                      ...
                      >
                        <span>{req.status === 'Resolved' ? 'View Details' : 'Resolve Inquiry'}</span>
