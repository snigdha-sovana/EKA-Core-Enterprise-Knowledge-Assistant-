...
export const OrionDocuments: React.FC = () => {
  const { documents } = useApp();
  const [searchQuery, setSearchQuery] = useState('');
  const [activeDoc, setActiveDoc] = useState<KnowledgeDocument | null>(null);
  const [selectedVersion, setSelectedVersion] = useState<DocumentVersion | null>(null);
  const [isCreatingRFC, setIsCreatingRFC] = useState(false);
  const [rfcNotice, setRfcNotice] = useState<string | null>(null);

  // Filter for Project Orion documents
  const orionDocs = documents.filter(d => d.department === 'Projects');

  const filtered = orionDocs.filter(doc => {
    const matchesSearch =
  ...
  const handleProposeRFC = () => {
    setRfcNotice('Technical RFC created: Architecture Spec v2.5-RFC (AWS IAM AssumeRole for Read-Replica Debugging).');
    setIsCreatingRFC(false);
    setTimeout(() => setRfcNotice(null), 5000);
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">
            Project Orion Architecture & Runbooks
          </h1>
          <p className="text-xs text-slate-500">
            System architecture designs, deployment runbooks, and UAT acceptance criteria indexed in EKA.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <button
            onClick={() => setIsCreatingRFC(true)}
            className="btn-erp-primary text-xs py-2 px-3.5 shadow-xs"
          >
            <Plus className="w-4 h-4" />
            <span>Create Technical RFC</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 enterprise-card p-4 bg-white">
        <div className="relative w-full sm:w-80">
          ...
          <input
            type="text"
            placeholder="Search architecture, Kafka, deployment SOPs..."
            value={searchQuery}
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          <span className="font-semibold text-slate-700">{filtered.length} Technical Specs</span>
          <span>•</span>
          <span className="text-emerald-600 font-semibold flex items-center gap-1">
            <Sparkles className="w-3.5 h-3.5" /> ChromaDB Vectorized
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filtered.map(doc => (
          <div
            ...
            className="enterprise-card p-5 bg-white border border-slate-200/80 hover:border-emerald-300 hover:shadow-md transition-all cursor-pointer flex flex-col justify-between group"
          >
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200 flex items-center gap-1">
                    <Lock className="w-2.5 h-2.5" /> Orion Only
                  </span>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="space-y-2">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  ...
                  Technical Specification Preview ({selectedVersion?.version || activeDoc.version})
                </h3>
          ...
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 max-w-lg w-full p-6 space-y-4 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-900">Create Technical RFC / Runbook</h3>
              </div>
              <button onClick={() => setIsCreatingRFC(false)} className="text-slate-400 hover:text-slate-600">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <label className="font-bold text-slate-700 block mb-1">Target Document</label>
                <select className="enterprise-input w-full">
                  <option>Project Orion Architecture Specification v2.4</option>
                  <option>Production Release & Canary Deployment Runbook v1.8</option>
                  <option>User Acceptance Testing (UAT) Milestone Plan v1.2</option>
                </select>
              </div>

              <div>
                <label className="font-bold text-slate-700 block mb-1">Target RFC Version</label>
                <input type="text" readOnly value="v2.5-RFC" className="enterprise-input w-full bg-slate-50 font-mono" />
              </div>

              <div>
                <label className="font-bold text-slate-700 block mb-1">Architectural Change Specification</label>
                <textarea
                  rows={3}
                  defaultValue="Define self-service AWS IAM AssumeRole runbook allowing verified engineers temporary read-replica access with 2-hour TTL for latency profiling."
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
              <button onClick={() => setIsCreatingRFC(false)} className="btn-secondary text-xs py-1.5 px-3">
                Cancel
              </button>
              <button onClick={handleProposeRFC} className="btn-erp-primary text-xs py-1.5 px-4 bg-emerald-600 hover:bg-emerald-700">
                Publish Technical RFC
              </button>
            </div>
          </div>
        </div>
