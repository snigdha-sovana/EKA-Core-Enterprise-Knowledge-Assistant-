...
async def seed_data():
    async for session in get_async_session():
        ...
        tenant = Tenant(tenant_id=tenant_id, name="Acme Corp (Simulator)", slug="acme-sim", status="active")
        session.add(tenant)
        ...
        doc2 = DocumentModel(
            ...
            chunk_count=5,
            status="active",
            access_policy={"roles": ["curator"], "user_ids": [str(dept_admin_id)], "is_public": False}
        )
